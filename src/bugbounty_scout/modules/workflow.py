...
def render_summary(manifest: WorkflowManifest, format: str = "markdown") -> str:
    ...
    lines = [
        ...
        *[f"- {item}" for item in manifest.summary.next_manual_actions],
        ...
        "",
        "## Manual submission packaging",
        "- Use `bbs submit from-evidence <workspace.yml>` for evidence workspaces.",
        "- Use `bbs submit from-lead <correlation.yml> --lead-id <id>` for correlator leads.",
        "- Use `bbs submit from-workflow <workspace>` for workflow outputs.",
        "- Packaging remains an explicit local command; reports are never auto-submitted.",
        "",
        "## Limitations",
